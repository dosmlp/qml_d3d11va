...
AVPixelFormat HWAccelFormat(AVFrame *frame)
{
...
DFFmpegVideoBuffer::DFFmpegVideoBuffer(AVFrameUPtr frame)
                               ...
                               { 1,1}))
{
    if (frame->hw_frames_ctx) {
        ...
        m_pixelFormat = toQtPixelFormat(HWAccelFormat(m_hwFrame.get()));
        return;
    }
...
DFFmpegVideoBuffer::~DFFmpegVideoBuffer() = default;

void DFFmpegVideoBuffer::convertSWFrame()
{
    Q_ASSERT(m_swFrame);
...
void DFFmpegVideoBuffer::setTextureConverter(const TextureConverter &converter)
{
...
QVideoFrameFormat::ColorSpace DFFmpegVideoBuffer::colorSpace() const
{
...
QVideoFrameFormat::ColorTransfer DFFmpegVideoBuffer::colorTransfer() const
{
...
QVideoFrameFormat::ColorRange DFFmpegVideoBuffer::colorRange() const
{
...
void DFFmpegVideoBuffer::setRhi(QRhi *rhi)
{
    m_textureConverter = TextureConverter(rhi);
    m_textureConverter.init(m_hwFrame.get());
    m_type = QVideoFrame::RhiTextureHandle;
}
inline std::optional<qreal> mul(qreal a, AVRational b)
{
    return b.den != 0 ? a * qreal(b.num) / qreal(b.den) : std::optional<qreal>{};
}
float DFFmpegVideoBuffer::maxNits()
{
    float maxNits = -1;
    for (int i = 0; i < m_frame->nb_side_data; ++i) {
        ...
        if (sd->type == AV_FRAME_DATA_MASTERING_DISPLAY_METADATA) {
            ...
            auto maybeLum = mul(10'000., data->max_luminance);
            if (maybeLum)
...
QAbstractVideoBuffer::MapData DFFmpegVideoBuffer::map(QVideoFrame::MapMode mode)
{
    if (!m_swFrame) {
    ...
    MapData mapData;
    auto *desc = textureDescription(pixelFormat());
...
std::unique_ptr<QVideoFrameTextures> DFFmpegVideoBuffer::mapTextures(QRhi *)
{
    if (m_textures)
        return {};
    if (!m_hwFrame)
...
quint64 DFFmpegVideoBuffer::textureHandle(QRhi *rhi, int plane) const
{
    return m_textures ? m_textures->textureHandle(rhi, plane) : 0;
}

QVideoFrameFormat::PixelFormat DFFmpegVideoBuffer::pixelFormat() const
{
    return m_pixelFormat;
}

QSize DFFmpegVideoBuffer::size() const
{
    return m_size;
}

QVideoFrameFormat::PixelFormat DFFmpegVideoBuffer::toQtPixelFormat(AVPixelFormat avPixelFormat, bool *needsConversion)
{
    if (needsConversion)
...
AVPixelFormat DFFmpegVideoBuffer::toAVPixelFormat(QVideoFrameFormat::PixelFormat pixelFormat)
{
    switch (pixelFormat) {
    default:
